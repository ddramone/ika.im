...
As you might imagine, managing positions of all these elements by simply hardcoding would be a nightmare.
...
We created `<View>` component to abstract yoga node creation. `<View>` can be nested to create complex layouts. Then we always use `<BrowserView>` component as a direct child of `<View>`. It calculates size and position of its parent `<View>` yoga node and applies it to the BrowserView instance in the main process sending them via ipc.
...
Special thankx to [Felix Rieseberg](https://felixrieseberg.com/) and [Sam Maddock](https://bsky.app/profile/samuelmaddock.com) for helping us to navigate through this maze.
